...
async function transaction<T>(
  ...
  const complete = new Promise<void>((resolve, reject) => {
    ...
    tx.onabort = () =>
      reject(tx.error ?? new Error('資料未儲存，請保留訂單後重試。'));
...
export async function openStore(
  ...
  try {
    await transaction(db, 'readwrite', async (tx) => {
      ...
      const initial: unknown = legacyRaw
        ? JSON.parse(legacyRaw)
        : { menu: defaultMenu, orders: [] };
...
/** Upgrade only the untouched previous sample. Never overwrite a custom menu,
 * reset the counter, or rewrite an order's historical price/name snapshot. */
export async function upgradeUntouchedMenu(
  db: IDBDatabase,
  previous: MenuItem[],
  next: MenuItem[],
): Promise<boolean> {
  validateMenu(next);
  const signature = (menu: MenuItem[]) =>
    JSON.stringify(
      menu.map((item) => [
        item.id,
        item.name,
  ...
  return transaction(db, 'readwrite', async (tx) => {
    ...
    if (
      settings.menuRevision !== 0 ||
      signature(settings.menu) !== signature(previous)
    )
      return false;
    store.put({
      ...settings,
      menu: structuredClone(next),
      menuRevision: 1,
      menuPristine: true,
    });
    return true;
...
export async function saveMenu(
  ...
  await transaction(db, 'readwrite', async (tx) => {
    const store = tx.objectStore('meta');
    const settings = await request<Meta>(store.get('settings'));
    if (settings.menuRevision !== expectedRevision)
      throw new Error('其他分頁已更新菜單。請關閉菜單後重新開啟再修改。');
    store.put({
      ...settings,
      menu,
      menuRevision: settings.menuRevision + 1,
      menuPristine: false,
    });
  });
}

export async function commitVoid(
  db: IDBDatabase,
  id: string,
  reason: string,
): Promise<void> {
  ...
  return transaction(db, 'readwrite', async (tx) => {
    ...
    for (const order of backup.orders) {
      ...
      if (found) {
        ...
        if (canonical(found) !== canonical(order))
          throw new Error(`訂單 ${order.id} 有不同版本，已取消整份匯入。`);
      } else {
        orders.add(order);
        added += 1;
        counter = Math.max(counter, order.number);
      }
    }
    const restoreMenu =
      existingOrders.length === 0 &&
      (settings.menuRevision === 0 || settings.menuPristine === true);
    meta.put({
      ...settings,
      counter,
      menu: restoreMenu ? backup.menu : settings.menu,
      menuRevision: restoreMenu
        ? settings.menuRevision + 1
        : settings.menuRevision,
      menuPristine: restoreMenu ? false : settings.menuPristine,
    });
    return added;
  });
}
