...
export default function Home() {
  ...
  async function refresh() {
    ...
    setMenuRevision(data.menuRevision);
  }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        await upgradeUntouchedMenu(connection, LEGACY_SAMPLE_MENU, WUJIE_MENU);
        await refresh();
        if (!cancelled) setReady(true);
      } catch (cause) {
        if (!cancelled)
          setError(
            `無法載入帳本。原資料已保留。${cause instanceof Error ? cause.message : ''}`,
          );
      }
    };
    void load();
    const channel =
      typeof BroadcastChannel !== 'undefined'
        ? new BroadcastChannel('smallshop-pos-v2')
        : null;
    channelRef.current = channel;
    const reload = () => {
      void refresh().catch(() =>
        setError('無法重新讀取帳本。請先備份並重新載入。'),
      );
    };
      ...
      context.registerTool(
        {
          ...
          execute(input) {
            if (busyRef.current || !dbRef.current)
              throw new Error('帳本尚未就緒或正在儲存。');
            ...
            const additions: CartLine[] = request.items.map((requested) => {
              const item = menu.find(
                (candidate) =>
                  candidate.id === requested.menuId && candidate.available,
              );
              ...
              if (
                requested.withAddon !== undefined &&
                typeof requested.withAddon !== 'boolean'
              )
                throw new Error('加料選項不正確。');
              if (requested.withAddon && !item.addon)
                throw new Error('這個餐點沒有加料選項。');
              const addonName = requested.withAddon
                ? item.addon?.name
                : undefined;
              return {
                key: JSON.stringify([
                ...
                unitPrice:
                  item.price + (addonName ? (item.addon?.price ?? 0) : 0),
                quantity,
                addonName,
              };
            });
              ...
              additions.forEach((addition) => {
                ...
                if (index >= 0)
                  next[index] = {
                    ...
                    quantity: Math.min(
                      999,
                      next[index].quantity + addition.quantity,
                    ),
                  };
  ...
  const visibleMenu = menu.filter(
    (item) =>
      ...
      item.name.includes(menuQuery.trim()),
  );
  ...
  const todayOrders = useMemo(
    () =>
      orders.filter(
        (order) => !order.voidedAt && dateKey(order.createdAt) === today,
      ),
    [orders, today],
  );
  ...
  function adjustQuantity(key: string, amount: number) {
    ...
    setCart((current) =>
      current
        .map((line) =>
          line.key === key
            ? { ...line, quantity: Math.min(999, line.quantity + amount) }
            : line,
        )
  ...
  function appendPaid(value: string) {
    if (busyRef.current) return;
    setPaidText((current) =>
      (current + value).replace(/^0+(?=\d)/, '').slice(0, 6),
    );
  }
  ...
  async function exportBackup() {
    ...
    try {
      ...
      download(
        ...
        `小店快收_完整備份_${today}.json`,
      );
      setNotice('備份檔已產生，請確認下載完成並另存一份到其他裝置。');
    } catch {
      setError('備份讀取失敗，請勿清除瀏覽器資料。');
    }
  }
    ...
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="topbar-actions">
          <button className="icon-action" onClick={() => setReportOpen(true)}>
          ...
          <button
            ...
            disabled={!ready || busy}
            onClick={() => {
              setDraftMenu(structuredClone(menu));
              setDraftRevision(menuRevision);
      ...
      <div className="data-banner">
        <span>
          展示帳本僅存此瀏覽器，尚未提供雲端備份及離線重啟。正式營業需使用另行驗收的版本。
        </span>
        <button disabled={!ready || busy} onClick={() => void exportBackup()}>
          完整備份
        </button>
        <label>
          匯入備份
          <input
            ...
            onChange={(event) => {
              ...
              void file
                ...
                .catch(() =>
                  setError('備份內容不完整或格式不正確，尚未更動任何資料。'),
                );
            }}
          />
        </label>
      ...
      {error && (
        <div className="data-message error" role="alert">
          {error}
          {!ready && (
            <button
              onClick={() => {
      ...
      <section className="workspace">
        <div className="menu-panel">
          <div className="pos-menu-intro">
            <div>
              <span className="neo-label">FRESH PICKS. HAPPY BITES.</span>
              <h2>今天，想吃點什麼？</h2>
              <p>五結大腸麵線菜單 · 點餐後可加料、算找零。</p>
            </div>
            <img
          ...
          <label className="pos-menu-search">
            ...
            <input
              aria-label="搜尋餐點"
              placeholder="搜尋餐點名稱，例如：大腸麵線"
              value={menuQuery}
              onChange={(e) => setMenuQuery(e.target.value)}
            />
            <span>{visibleMenu.length} 品項</span>
          </label>
              ...
              >
                ...
                >
                  <span className="menu-category">
                    {item.category}
                    <span className="pos-food-icon" aria-hidden="true">
                      {item.category === '煎類' ? (
                        <EggFried />
                      ) : item.category === '小點' ? (
                        <Cookie />
                ...
                {item.addon && (
                  ...
                  >
                    ...
                    {item.addon.name} {money(item.addon.price)}
                  </button>
                )}
              </article>
            ))}
        ...
        <aside className="cart-panel">
              ...
              cart.map((line) => (
                <div className="cart-line" key={line.key}>
                  <div className="cart-line-copy">
                    ...
                    <span>{money(line.unitPrice)} / 份</span>
                  </div>
          ...
          <div className="checkout-panel">
            ...
            <div className="payment-row">
              <div>
                <span>收現</span>
                <strong>{paidText ? money(paid) : '直接結帳'}</strong>
              </div>
              <div className={change < 0 ? 'negative' : ''}>
                <span>找零</span>
                ...
                <button
                  ...
                  onClick={() =>
                    key === '清除'
                      ? setPaidText('')
                      : key === '←'
                        ? setPaidText((value) => value.slice(0, -1))
                        : appendPaid(key)
                  }
                >
                  {key}
            ...
            <div className="checkout-actions">
              ...
              <button
                ...
                disabled={
                  !ready || busy || !cart.length || (!!paidText && paid < total)
                }
                onClick={checkout}
              >
                {busy ? '儲存中…' : '完成結帳'}
      ...
      <Dialog open={reportOpen} onOpenChange={setReportOpen}>
        <DialogContent className="report-dialog">
          ...
          <Tabs defaultValue="today">
            <TabsList>
              <TabsTrigger value="today">今日摘要</TabsTrigger>
            ...
            <TabsContent value="today" className="report-content">
              ...
              <div className="report-table">
                ...
                {itemSales.length ? (
                  ...
                  <p className="empty-report">今天還沒有訂單。</p>
                )}
              </div>
            </TabsContent>
                  ...
                  orders.map((order) => (
                    <article key={order.id} className="history-card">
                      <div>
                        ...
                        <span>
                          {timeLabel(order.createdAt)}・{order.diningType}
                        </span>
                      </div>
                      ...
                      <strong>
                        {order.voidedAt ? '已作廢' : money(order.total)}
                      </strong>
                      <button
                        aria-label={`列印訂單 ${order.number}`}
                        onClick={() => printReceipt(order)}
                      >
                        <Printer />
                      </button>
                      {order.voidedAt ? (
                        <small>作廢原因：{order.voidReason}</small>
                      ) : (
                        <button
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent className="settings-dialog">
          <DialogHeader>
            <DialogTitle>菜單管理</DialogTitle>
            <DialogDescription>
              改完後按「儲存菜單」。已加入購物車的品項保留原價格，後續加入使用新價格。
            </DialogDescription>
          </DialogHeader>
          <div className="pos-restore-menu">
            <span>原工具菜單：21 品項 · 煎類加蛋 +$10</span>
            <button
              className="class-btn"
              onClick={() => setRestoreMenuOpen(true)}
            >
              套用五結菜單
            </button>
          </div>
          ...
          <div className="data-actions">
            ...
            <button
              ...
              onClick={() =>
                void mutate(async (db) => {
                  draftMenu.forEach((item) => amount(item.price));
                  await saveMenu(db, draftMenu, draftRevision);
                  setCategory('全部');
                  setMenuQuery('');
      ...
      <Dialog open={checkoutOpen} onOpenChange={setCheckoutOpen}>
        <DialogContent className="checkout-dialog">
          <DialogHeader>
            <DialogTitle>結帳完成</DialogTitle>
            <DialogDescription>訂單已儲存到這台裝置。</DialogDescription>
          </DialogHeader>
          {lastOrder && (
            <div className="checkout-success">
              ...
              <span>{lastOrder.diningType}・取餐號</span>
              <strong>#{String(lastOrder.number).padStart(3, '0')}</strong>
              <div className="success-total">
                <span>找零</span>
                <strong>{money(lastOrder.paid - lastOrder.total)}</strong>
              </div>
      ...
      <AlertDialog open={restoreMenuOpen} onOpenChange={setRestoreMenuOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>套用五結大腸麵線菜單？</AlertDialogTitle>
            <AlertDialogDescription>
              將以原工具的 21
              品項與價格取代目前的菜單草稿。歷史訂單與購物車不會變更。按「儲存菜單」後才正式套用。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>保留目前菜單</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => setDraftMenu(structuredClone(WUJIE_MENU))}
            >
              套用至草稿
            </AlertDialogAction>
          </AlertDialogFooter>
      ...
      <AlertDialog open={clearOpen} onOpenChange={setClearOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>清空目前訂單？</AlertDialogTitle>
            <AlertDialogDescription>
              購物車內的餐點會全部移除。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>保留訂單</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
                setCart([]);
                setPaidText('');
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>匯入完整備份？</AlertDialogTitle>
            <AlertDialogDescription>
              備份含 {backup?.orders.length} 筆訂單、{backup?.menu.length}{' '}
              個品項。只補入缺少的訂單；若同一 ID
              內容衝突，整份取消。菜單只在全新帳本還原。請先下載目前的完整備份。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                ...
                if (input)
                  void mutate(async (db) => {
                    const count = await mergeBackup(db, input);
                    setBackup(null);
                    setNotice(`備份匯入完成，新增 ${count} 筆訂單。`);
                  });
              }}
            >
              確認匯入
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>作廢訂單 #{voidTarget?.number}</DialogTitle>
            <DialogDescription>
              保留原單與原因，並從原訂單日的有效營收扣除。此功能不會實際退款，不適用部分退款。
            </DialogDescription>
          </DialogHeader>
          <label>
            作廢原因
          ...
          <button
            ...
            onClick={() => {
              ...
              if (target)
                void mutate(async (db) => {
                  await commitVoid(db, target.id, voidReason);
                  setVoidTarget(null);
                  setNotice('訂單已作廢，原始紀錄已保留。');
                });
            }}
          >
            確認作廢
      ...
      {ready &&
        ...
        createPortal(
          <div id="print-area">
            <h1>小店快收・展示單據</h1>
            <p>
              {printOrder.diningType}・取餐號 {printOrder.number}
              {printOrder.voidedAt ? '（作廢）' : ''}
            </p>
            <hr />
            {printOrder.lines.map((line) => (
              <div className="print-line" key={line.key}>
                <span>
            ...
            <p>
              收現 {money(printOrder.paid)}・找零{' '}
              {money(printOrder.paid - printOrder.total)}
            </p>
            <small>{timeLabel(printOrder.createdAt)}・非統一發票</small>
          </div>,
          document.body,
        )}
      {!ready && !error && (
